...
Tetrish. The root `CMakeLists.txt` builds the reusable libraries. The
`draw_app` application is maintained in its own repository and consumes the
`tui` and `config` targets from this project.
